  ...
  // if (command === 'serve') {
    return {
      ...
      server: {
        port: 443,
        https: {
          cert: readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), './.cert/tonscriber.pem')),
          key: readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), './.cert/tonscriber-key.pem')),
        },
        host: 'tonscriber.local',
      },
      publicDir: "./public",
    }
  // } else {
